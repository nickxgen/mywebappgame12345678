...
# Webcam Overlay (Bottom-Right, matching HTML layout)
CAM_WIDTH = 320
CAM_HEIGHT = 240
...
# Gear limits matching HTML logic
GEAR_LIMITS = {1: 6, 2: 13, 3: 22}

# --- IMAGE FILTERS (from Python codebase) ---
...
# --- GESTURE RECOGNITION (from HTML logic) ---
...
pygame.display.set_caption("3D Supercar - Gesture Gears & Camera Filters")
...
# 3D Pseudo-Road Grid Environment
trees = []
...
while running:
    ...
    if ret:
        ...
        # Steering Calculation (Two-Hand Tilt or Single Hand)
        ...
        if found_right:
            target_speed = 0.0 if right_open else float(GEAR_LIMITS[current_gear])
    ...
    if not hand_detected:
        target_speed = 2.0  # Idle Coasting
    ...
    car_x = np.clip(car_x, 400, SCREEN_WIDTH - 400)

    score += int(current_speed / 4)

    # 3. 3D PERSPECTIVE RENDERING
    screen.fill((26, 43, 60)) # Deep background color matching Three.js scene
    ...
    # Move and Draw Dynamic Environment Trees
    for tree in trees:
    ...
    # Draw 3D-styled Supercar Chassis
    car_y = SCREEN_HEIGHT - 130
    car_w, car_h = 90, 110
    
    # Body shadow & lower chassis
